...
import RandomImagesList from './components/RandomImagesList'
...
class App extends Component {
  state = {
    ...
    seconds: 10,
    score: 0,
    randomGenratedImage: imagesList[0].imageUrl,
    imageIdClicked: false,
    generateImageId: imagesList[0].id,
    showTabImages: tabsList[0].tabId,
  }
  ...
  onClickImage = id => {
    ...
    const randomImages = imagesList[randomNumber].imageUrl
    const randomImageId = id

    this.setState(prevState => ({
      imageIdClicked: !prevState.imageIdClicked,
      generateImageId: imagesList[randomNumber].id,
      randomGenratedImage: randomImages,
    }))
    if (generateImageId === randomImageId) {
      this.setState(prevState => ({
  ...
  showTabClickedImages = tabId => {
    const {showTabImages} = this.state
    this.setState({
      showTabImages: tabId,
    })
  }
  ...
  playAgain = () => {
    this.setState(prevState => ({
      isTimeStarted: !prevState.isTimeStarted,
      seconds: 10,
      score: 0,
      randomGenratedImage: imagesList[0].imageUrl,
      imageIdClicked: false,
    ...
    return (
      <div className="score-card">
        ...
        <div className="original-score">
          <h1 className="match-score">{score}</h1>
        </div>
        <div className="play-again-button">
          <button
            type="button"
            className="play-back-button"
  ...
  render() {
    ...
    const filterTabImages = imagesList.filter(
      eachTabImage => eachTabImage.category === showTabImages,
    )
    const renderScoreCard = this.scoreCard()
    ...
    return (
      <div className="app-container">
        ...
        {seconds === 0 ? (
          ...
          <div className="random-images-list">
            <img src={randomGenratedImage} alt="match" className="match" />
              ...
              <ul className="list-tabs">
                {tabsList.map(eachTab => (
                  <TabsList
                    key={eachTab.tabId}
                    tabsList={eachTab}
                    showTabClickedImages={this.showTabClickedImages}
                  />
                ))}
              </ul>
            </div>
